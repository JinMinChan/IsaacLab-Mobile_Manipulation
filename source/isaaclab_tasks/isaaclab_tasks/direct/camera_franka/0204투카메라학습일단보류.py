...
class CameraFrankaEnvCfg(DirectRLEnvCfg):
    ...
    camera_top = CameraCfg(
        data_types=["rgb"],
        prim_path="/World/envs/env_.*/Robot/summit_xl_top_structure/Camera",  # 기존 카메라 경로
        height=320,
        width=320,
        update_period=0.1,
    ...
    camera_hand = CameraCfg(
        ...
        offset=CameraCfg.OffsetCfg(
            pos=(0.05, 0.0, 0.0),           # 예시로 약간 앞으로 뺀 위치
            rot=(0.0, 0.7071, 0.0, 0.7071), # 예시: 90도 회전
            convention="world",
        ),
    )
...
class CameraFrankaEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: CameraFrankaEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.valve_detected_results_top = [False] * self.cfg.scene.num_envs
        self.valve_detected_results_hand = [False] * self.cfg.scene.num_envs
    ...
    def _setup_scene(self):
        ...
        # 씬 설정 후 카메라 초기화
        super()._setup_scene()
        self.camera_top = Camera(self.cfg.camera_top)
        self.camera_hand = Camera(self.cfg.camera_hand)
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        for env_id in env_ids:
            self.valve_detected_results_hand[env_id] = False
            self.valve_detected_results_top[env_id] = False
    ...
    def _compute_intermediate_values(self, env_ids: torch.Tensor | None = None):
        ...
        hand_not_detected = ~torch.tensor(self.valve_detected_results_hand, device=self.device, dtype=torch.bool)
        self.robot_dof_targets[hand_not_detected, 3:10] = 0.0
        # Z 축 차이 계산
        ...
        #hand_detected_tensor = torch.tensor(self.valve_detected_results_hand, device=self.device)
        #hand_detected_sub = hand_detected_tensor[env_ids]  # shape: [len(env_ids)]

        # 10) mask도 env_ids 기준으로 서브셋 추출
        #mask_sub = mask[env_ids]  # shape: [len(env_ids)]

        # 11) detect_sub = (~mask_sub) & (~hand_detected_sub)
        #detect_sub = (~mask_sub) & (~hand_detected_sub)

        # 12) detect_sub가 True인 env들에 대해 DOF 업데이트
        #     -> env_ids 중 detect_sub가 True인 인덱스만 뽑아 업데이트
        #self.robot_dof_targets[env_ids[detect_sub], 3:7] = 0.0
    ...
    def _compute_rewards(
        ...
        # ---------------------------
        # (1) YOLO 배치 추론 (검출 주기마다 수행)
        # ---------------------------
        DETECTION_INTERVAL = 12
        if self.timestep % DETECTION_INTERVAL == 0:
            # --- Top 카메라 ---
            # 현재까지 Top 카메라에서 밸브가 검출되지 않은 환경 인덱스를 한꺼번에 추출
            top_detected_tensor = torch.tensor(self.valve_detected_results_top, device=self.device, dtype=torch.bool)
            undetected_top_indices = (~top_detected_tensor).nonzero(as_tuple=True)[0].tolist()
            if undetected_top_indices:
                self._perform_yolo_detection(self.camera_top, self.valve_detected_results_top, undetected_top_indices)

            # --- Hand 카메라 ---
            # 조건: Top에서 밸브가 검출되었으면서, Hand에서는 아직 검출되지 않았고, Mobile Phase가 아닐 때
            top_tensor = torch.tensor(self.valve_detected_results_top, device=self.device, dtype=torch.bool)
            hand_tensor = torch.tensor(self.valve_detected_results_hand, device=self.device, dtype=torch.bool)
            # is_mobile_phase는 이전에 계산되어 있는 torch.Tensor라고 가정합니다.
            hand_needed_mask = top_tensor & (~hand_tensor) & (~is_mobile_phase)
            undetected_hand_indices = hand_needed_mask.nonzero(as_tuple=True)[0].tolist()
            if undetected_hand_indices:
                self._perform_yolo_detection(self.camera_hand, self.valve_detected_results_hand, undetected_hand_indices)

        # self.valve_detected_results 중 True/False를 텐서로
        valve_detected_top_tensor = torch.tensor(self.valve_detected_results_top, device=self.device)
        valve_detected_hand_tensor = torch.tensor(self.valve_detected_results_hand, device=self.device)
        ...
        # * yolo_top_reward
        #   - 예: top으로 탐지 성공했거나, mobile phase 아니면 +1, 그 외 -1
        #   - 사용자 정의 가능
        yolo_top_reward = torch.where(
            valve_detected_top_tensor | (~is_mobile_phase), 
            torch.tensor(1.0, device=self.device), 
            torch.tensor(-1.0, device=self.device)
        )

        # * yolo_hand_reward
        #   - 예: hand로 탐지 성공했으면 +2, 아니면 -1 ...
        #   - 원하는 로직에 맞춰 조정 가능
        yolo_hand_reward = torch.where(
            valve_detected_hand_tensor,
        ...
        axis4 = self.axis4  # (num_envs, 3) from _compute_intermediate_values
        ...
        self.extras["log"] = {
            ...
            "yolo_top_reward": yolo_top_reward.mean().item(),
            "yolo_hand_reward": yolo_hand_reward.mean().item(),
        ...
        #print("yolo_top_reward",yolo_top_reward)
        #print("yolo_hand_reward",yolo_hand_reward)
        return rewards
    
    def _perform_yolo_detection(self, camera: Camera, detection_results: list[bool], env_indices: list[int]) -> None:
        """
        주어진 카메라로 env_indices에 해당하는 환경들에 대해 YOLO 배치 추론을 수행하고,
        해당 인덱스의 detection_results 리스트를 업데이트합니다.
        """
        # 카메라 업데이트 (한 번만 호출)
        camera.update(self.dt * 12)
        rgb = camera.data.output.get("rgb", None)
        if rgb is None:
            return
        # (num_envs, H, W, 3) 형태로 변환 (알파 채널 무시)
        rgb_np = rgb[..., :3].cpu().numpy()

        # env_indices에 해당하는 이미지들을 BGR로 변환하여 리스트에 저장
        batch_imgs = [cv2.cvtColor(rgb_np[i], cv2.COLOR_RGB2BGR) for i in env_indices]

        # YOLO 배치 추론
        results = self.yolo_model.predict(batch_imgs, conf=0.8, verbose=False)

        # 결과 업데이트: 각 환경에 대해 'valve' 클래스가 일정 신뢰도 이상이면 True로 설정
        for env_id, result in zip(env_indices, results):
