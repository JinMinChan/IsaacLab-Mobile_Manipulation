...
class CameraFrankaEnv(DirectRLEnv):
    ...
    def _compute_rewards(
        ...
        valve_detected = torch.tensor(valve_detected, device=self.device)
        ...
        # ** Valve 탐지 보상 **
        valve_detect_reward = valve_detected.float() * 5.0

        # ** 최종 보상 계산 **
        rewards = mobile_distance_reward + valve_detect_reward

        # ** 로그 업데이트 **
        self.extras["log"] = {
            "mobile_distance_reward": mobile_distance_reward.mean(),
            "valve_detect_reward": valve_detect_reward.mean(),
