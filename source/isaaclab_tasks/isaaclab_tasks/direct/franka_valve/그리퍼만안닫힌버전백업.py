...
class FrankaValveEnv(DirectRLEnv):
    ...
    def _compute_rewards(
        ...
        finger_reward_open = torch.where(d > 0.025, finger_distance * 10, torch.zeros_like(finger_distance))

        # 손가락 간 거리 보상 계산 (0.08보다 작아질수록 점수 증가, 최대 4점)
        max_distance = 0.08
        finger_reward_close = torch.where(
            (d <= 0.025) & (dot1 > 0.5) & (dot2 > 0.5),
        ...
        finger_grip_bonus = torch.where(
            (d <= 0.025) & (finger_distance < 0.05),  # 손가락 간 거리가 충분히 줄어들면 보상 부여
            2.0 * (0.03 - finger_distance),  # 거리 감소에 비례한 보상
            torch.zeros_like(finger_distance)
        )
