...
class CameraFrankaEnv(DirectRLEnv):
    ...
    def detect_valve_with_yolo(self):
        """YOLO로 밸브 탐지."""
        # 카메라 이미지 업데이트
        self.camera.update(self.dt)
        if self.camera.data.output["rgb"] is not None:
            # 이미지 가져오기
            rgb_image = self.camera.data.output["rgb"][0, ..., :3].cpu().numpy()
    ...
    def _compute_rewards(
        ...
        # YOLO를 통해 밸브 탐지 여부 확인
        valve_detected = torch.tensor([self.detect_valve_with_yolo() for _ in range(num_envs)], device=self.device)

        # 밸브 탐지 시 10점 부여
        rewards = torch.where(valve_detected, 10.0, 0.0)
