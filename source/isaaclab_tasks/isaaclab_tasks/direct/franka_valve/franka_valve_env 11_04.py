...
class FrankaValveEnvCfg(DirectRLEnvCfg):
    ...
    # 보상 스케일
    dist_reward_scale = 10.0      # 거리 기반 보상
    rot_reward_scale = 5.0        # 정렬 보상
    grip_reward_scale = 4.0       # 파지 시도 보상
    action_penalty_scale = 0.02   # 행동 페널티
    turn_reward_scale = 10.0      # 밸브 회전 보상
    grip_reward_scale = 2.0       # 그립 보상 스케일 추가
    stability_reward_scale = 0.7
...
class FrankaValveEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: FrankaValveEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # 0.35 ,0 ,0.655
        valve_local_grasp_pose = torch.tensor([-0.145, 0.15, 0.00, 1.0, 0.0, 0.0, 0.0], device=self.device)  # 중심 위치
        ...
        self.valve_inward_axis = torch.tensor([-1, 0, 0], device=self.device, dtype=torch.float32).repeat(
            (self.num_envs, 1)
        )
    ...
    def _setup_scene(self):
        self.cfg.valve.init_state.pos = (0.2, 0, 0.8)  # 위치 고정
        self.cfg.valve.init_state.rot = (0.707, 0, 0.707, 0)  # x축을 기준으로 90도 회전
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # valve의 현재 회전 각도 가져오기 (valve_handle_joint)
        valve_dof_pos = self._valve.data.joint_pos  # 밸브의 모든 자유도(DoF) 각도

        # 보상 계산 호출
        return self._compute_rewards(
            actions=self.actions,
            valve_dof_pos=valve_dof_pos,  # 추가된 매개변수
            ...
            valve_up_axis=self.valve_up_axis,
            num_envs=self.num_envs,
            dist_reward_scale=self.cfg.dist_reward_scale,
            rot_reward_scale=self.cfg.rot_reward_scale,
            turn_reward_scale=self.cfg.turn_reward_scale,
            action_penalty_scale=self.cfg.action_penalty_scale,
            finger_reward_scale=self.cfg.grip_reward_scale,  # 손가락 보상 스케일 사용
            grip_threshold=self.cfg.grip_threshold
        )
    ...
    def _compute_rewards(
        ...
        axis4 = tf_vector(valve_grasp_rot, valve_up_axis)
        ...
        # 회전도 보상 (정확히 한 바퀴 돌리기)
        target_angle = 2 * torch.pi  # 한 바퀴 (360도)
        current_angle = valve_dof_pos[:, 0]  # 현재 밸브 회전 각도 (라디안)
        
        # 한 바퀴에 가까울수록 보상을 주고, 넘어가면 패널티를 주는 보상 계산
        turn_reward = torch.where(
            current_angle <= target_angle,
            (current_angle / target_angle) * turn_reward_scale,
            -turn_reward_scale * (current_angle - target_angle)
        )
        
        # 손가락 거리 패널티
        lfinger_dist = franka_lfinger_pos[:, 2] - valve_grasp_pos[:, 2]
        rfinger_dist = valve_grasp_pos[:, 2] - franka_rfinger_pos[:, 2]
        ...
        self.extras["log"] = {
            ...
            "turn_reward": turn_reward.mean(),
            "finger_dist_penalty": finger_dist_penalty.mean(),
            "action_penalty": (-action_penalty_scale * action_penalty).mean(),
        }
        #print("axis1:", axis1[0].cpu().numpy()) 
        #print("axis2:", axis2[0].cpu().numpy()) 
        #print("axis3:", axis3[0].cpu().numpy()) 
        #print("axis4:", axis4[0].cpu().numpy()) 

        #print("valve_grasp_rot:", valve_grasp_rot[0].cpu().numpy())
        #print("valve_inward_axis:", valve_inward_axis[0].cpu().numpy())
        #print("valve_up_axis:", valve_up_axis[0].cpu().numpy())
        return rewards
