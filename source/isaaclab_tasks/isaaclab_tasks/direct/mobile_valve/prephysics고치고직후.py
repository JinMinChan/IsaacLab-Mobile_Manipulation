...
class MobileValveEnv(DirectRLEnv):
    ...
    def _compute_intermediate_values(self, env_ids: torch.Tensor | None = None):
        ...
        # Valve와 Grasp 간의 3D 벡터 계산
        full_vector = valve_pos - valve_grasp_pos

        # X 좌표를 0으로 설정하여 YZ 평면에 투영
        look_at_vector_yz = torch.cat((torch.zeros_like(full_vector[:, :1]), full_vector[:, 1:]), dim=-1)

        # 벡터 정규화
        look_at_vector_yz = look_at_vector_yz / torch.norm(look_at_vector_yz, p=2, dim=-1, keepdim=True)

        # axis4 업데이트
        self.axis4 = look_at_vector_yz
    ...
    def _compute_rewards(
        ...
        # 1. Mobile의 거리 유지 보상
        mobile_target_distance = torch.norm(robot_base_pos[:, :2] - valve_base_pos[:, :2], p=2, dim=-1)
        mobile_distance_reward = torch.where(
            (mobile_target_distance >= 0.8) & (mobile_target_distance <= 1.2),
            1.0 - torch.abs(mobile_target_distance - 1.0),  # 적절한 거리 유지
            -1.0 * torch.abs(mobile_target_distance - 1.0),  # 거리 초과/부족 패널티
        )
        ...
        franka_distance_reward = 2.0 / (1 + d)  # 거리 보상 강화

        # 4. 행동 패널티
        mobile_action_penalty = torch.sum(actions[:, :3]**2, dim=-1) / actions[:, :3].shape[-1]
        franka_action_penalty = torch.sum(actions[:, 3:10]**2, dim=-1) / actions[:, 3:10].shape[-1]
        action_penalty = mobile_action_penalty * 1.5 + franka_action_penalty

        # 5. 간소화된 손가락 보상
        close_condition = d < 0.025  # 목표 근처 조건
        finger_distance = torch.norm(robot_lfinger_pos - robot_rfinger_pos, p=2, dim=-1)
        finger_reward = torch.where(
            close_condition,                # 목표 근처(d < 0.025)일 때
            1.0 - finger_distance * 50.0,   # 손가락 닫기 보상 (손가락 거리가 작을수록 보상)
            finger_distance * 25.0          # 목표와 멀리 있을 땐 손가락 벌리기 보상
        )

        # 6. 총 보상 계산
        rewards = (
            + mobile_distance_reward  # Mobile의 거리 유지 보상
            + rot_reward              # Franka의 축 정렬 보상
            + franka_distance_reward  # Franka의 거리 보상
            - action_penalty          # 행동 제어 패널티
            + finger_reward           # 수정된 손가락 보상
        )
        ...
        self.extras["log"] = {
            ...
            "finger_reward": finger_reward.mean(),  # 수정된 손가락 보상 로깅
            "d": d.mean(),
        }
        ...
        #print("d:", d.cpu().numpy())
        #print("dot1:", dot1.cpu().numpy())
        #print("dot2:", dot2.cpu().numpy())
        # 액션 벡터 확인
