...
class CameraFrankaEnv(DirectRLEnv):
    ...
    def _compute_rewards(
        ...
        # YOLO를 일정 주기(예: 12스텝)마다 실행
        DETECTION_INTERVAL = 12

        # 현재 타임스텝이 12로 나누어떨어질 때만 YOLO 호출
        if self.timestep % DETECTION_INTERVAL == 0:
            for env_id in range(num_envs):
                # 이미 valve_detected_results[env_id] == True 이면 재검사 안 함
                if not self.valve_detected_results[env_id]:
                    detected = self.detect_valve_with_yolo(env_id)
                    if detected:
                        # 일단 한 번 True가 되면 그대로 유지
                        self.valve_detected_results[env_id] = True

        # 이 시점에서 self.valve_detected_results 안에는
        # "한번이라도 True 면 계속 True" 상태가 유지됨.
