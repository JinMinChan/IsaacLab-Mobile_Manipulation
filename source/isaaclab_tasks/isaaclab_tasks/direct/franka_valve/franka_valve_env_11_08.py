...
class FrankaValveEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: FrankaValveEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        robot_local_pose_pos += torch.tensor([0, 0.0, 0.02], device=self.device)
    ...
    def _compute_rewards(
        ...
        axis4 = self.axis4  # 업데이트된 look_at_vector 사용
        ...
        # reward for matching the orientation of the hand to the valve (fingers wrapped)
        ...
        # 거리 보상 조건 수정
        dist_reward = torch.where((rot_reward > 0.9) & (d < 0.001), dist_reward * 10, dist_reward)
        dist_reward = torch.where((rot_reward > 0.9) & (d <= 0.04) & (d >= 0.001), dist_reward * 5, dist_reward)
        dist_reward = torch.where((rot_reward > 0.9) & (d > 0.04) & (d <= 0.05), dist_reward * 3, dist_reward)
        dist_reward = torch.where((rot_reward > 0.9) & (d > 0.05) & (d <= 0.1), dist_reward * 2, dist_reward)


        # 손가락 간 거리 계산
        finger_distance = torch.abs(franka_lfinger_pos[:, 0] - franka_rfinger_pos[:, 0])

        # 손가락 보너스: d > 0.001일 때는 멀수록 보상, d <= 0.001일 때는 가까울수록 보상
        finger_bonus_open = torch.where(d > 0.001, finger_distance * 10, torch.zeros_like(finger_distance))
        finger_bonus_close = torch.where(d <= 0.001, (0.01 - finger_distance) * 20, torch.zeros_like(finger_distance))
        finger_bonus = torch.clamp(finger_bonus_open + finger_bonus_close, max=5)

        # 손가락 패널티 계산
        lfinger_dist = valve_grasp_pos[:, 0] - franka_lfinger_pos[:, 0]
        rfinger_dist = franka_rfinger_pos[:, 0] - valve_grasp_pos[:, 0]
        finger_dist_penalty = torch.zeros_like(lfinger_dist)
        finger_dist_penalty += torch.where(lfinger_dist < 0, lfinger_dist, torch.zeros_like(lfinger_dist))
        finger_dist_penalty += torch.where(rfinger_dist < 0, rfinger_dist, torch.zeros_like(rfinger_dist))

        # 밸브 회전 보상 및 패널티
        valve_rotation = valve_dof_pos[:, 0]
        valve_rotation_reward = torch.where(
            (rot_reward > 0.9) & (finger_distance < 0.0005),
            (10 * (valve_rotation / (2 * torch.pi))).clamp(max=10),
            torch.zeros_like(valve_rotation)
        )
        valve_rotation_penalty = torch.where(
            valve_rotation > 2 * torch.pi,
            -10 * torch.ones_like(valve_rotation),
            torch.zeros_like(valve_rotation)
        )
        valve_rotation_reward += valve_rotation_penalty
        ...
        self.extras["log"] = {
            ...
            "finger_bonus_open": finger_bonus_open.mean(),
            "finger_bonus_close": finger_bonus_close.mean(),
            "finger_bonus_total": finger_bonus.mean(),
            "finger_dist_penalty": (finger_reward_scale * finger_dist_penalty).mean(),
            "valve_rotation_reward": valve_rotation_reward.mean(),
            "valve_rotation_penalty": valve_rotation_penalty.mean(),
            "valve_rotation_total": valve_rotation_reward.mean(),
        }
        #elf.visualize_valve_grasp_pos()
        #self.visualize_axes()
    ...
    def visualize_valve_grasp_pos(self):
        ...
        # 두 손가락의 중간 지점 대신 self.robot_grasp_pos 사용
