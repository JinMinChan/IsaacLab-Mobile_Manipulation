...
class MobileValveEnvCfg(DirectRLEnvCfg):
    ...
    base_position_min = torch.tensor([-10.0, -10.0, -360.0], device='cpu')  # CPU를 사용하는 경우
    base_position_max = torch.tensor([10.0, 10.0, 360.0], device='cpu')
...
class MobileValveEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: MobileValveEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.base_position_min = self.cfg.base_position_min.to(self.device)
        self.base_position_max = self.cfg.base_position_max.to(self.device)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        # 액션 클램핑 (위치 명령을 범위에 맞게 조정)
        self.actions = actions.clone().clamp(-1.0, 1.0)

        valve_base_pos = self._valve.data.body_pos_w[:, self.valve_link_idx]
        # 1. 밸브의 목표 위치 계산 (0.8m 떨어진 위치)
        #target_position = valve_base_pos[:, :2] - torch.tensor([0.8, 0.0], device=valve_base_pos.device)
        target_position = torch.tensor([4.0, 0.0], device=valve_base_pos.device).unsqueeze(0).repeat(valve_base_pos.size(0), 1)

        # 2. 목표 위치를 robot_dof_targets에 적용
        self.robot_dof_targets[:, :2] = target_position

        # 3. Z축은 고정된 값으로 설정 (예: 0으로 고정)
        self.robot_dof_targets[:, 2] = 0.0  # 또는 다른 고정값

        # 4. 매니퓰레이터 및 그립퍼 제어 (기존 액션 처리)
        ...
        # 로봇에 적용
        self._robot.set_joint_position_target(self.robot_dof_targets[:, :3], joint_ids=[0, 1, 2])  # 베이스 위치
        self._robot.set_joint_position_target(self.robot_dof_targets[:, 3:10], joint_ids=list(range(3, 10)))  # 매니퓰레이터
        self._robot.set_joint_position_target(self.robot_dof_targets[:, 10:12], joint_ids=[10, 11])  # 그립퍼
    ...
    def _compute_rewards(
        ...
        # 1. 모바일 베이스와 밸브 간 거리 계산 (x, y만 사용)
        base_dist = torch.norm(robot_base_pos[:, :2] - valve_base_pos[:, :2], p=2, dim=-1)

        # 2. base_dist가 0.81 이하일 때만 Franka 관련 보상 함수 계산
        action_penalty = torch.sum(actions[:, 3:]**2, dim=-1)  # 0, 1, 2 제외
        # 축 정렬 계산
        ...
        dot1 = torch.bmm(axis1.view(-1, 1, 3), axis2.view(-1, 3, 1)).squeeze(-1).squeeze(-1)
        dot2 = torch.bmm(axis3.view(-1, 1, 3), axis4.view(-1, 3, 1)).squeeze(-1).squeeze(-1)
        ...
        # 6. 총 보상 계산
        rewards = torch.where(base_dist > 0.81, 
            -action_penalty_scale * action_penalty,  # dist > 0.81일 때는 패널티만 적용
            (
                rot_reward_scale * rot_reward         # 축 정렬 보상
                + dist_reward_scale * dist_reward     # 거리 보상
                - action_penalty_scale * action_penalty  # 행동 제어 패널티
            )
        )
        ...
        self.extras["log"] = {
            ...
            "rewards": rewards.mean()  # base_dist도 로깅
        }
        ...
        #print("robot_base_pos:", robot_base_pos.cpu().numpy())
        print("d:", d.cpu().numpy())
        # 액션 벡터 확인
