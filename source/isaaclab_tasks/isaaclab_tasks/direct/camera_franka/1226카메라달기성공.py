...
class CameraFrankaEnvCfg(DirectRLEnvCfg):
    ...
    robot = ArticulationCfg(
        ...
        spawn=sim_utils.UsdFileCfg(
            usd_path="/home/vision/Downloads/floating_franka_original_camera.usd",  # FLOATING_FRANKA USD 경로
...
class CameraFrankaEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: CameraFrankaEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # 환경별 카메라 초기화
        for i in range(self.cfg.scene.num_envs):  # 환경 수만큼 반복
            camera_path = f"/World/envs/env_{i}/Robot/panda_hand/Camera"
            
            if not prim_utils.is_prim_path_valid(camera_path):
                # 프림이 없으면 생성
                self.robot_camera = Camera(cfg=self.cfg.camera)
                self.robot_camera.set_prim_path(camera_path)
                print(f"Camera created at path: {camera_path}")
            else:
                # 프림이 있으면 재사용 (기존 프림을 바인딩만 함)
                print(f"Camera already exists at path: {camera_path}, reusing it.")
                self.robot_camera = CameraCfg(
                    prim_path=camera_path,
                    update_period=self.cfg.camera.update_period,
                    height=self.cfg.camera.height,
                    width=self.cfg.camera.width,
                    data_types=self.cfg.camera.data_types,
                    spawn=None,  # 중복 생성 방지
                    offset=self.cfg.camera.offset,
                )
